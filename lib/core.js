...
export function compileFilter(pattern) {
  ...
  try {
    ...
    throw new Error(`hydrophone: invalid pattern ${JSON.stringify(pattern)}: ${error instanceof Error ? error.message : String(error)}`)
  }
}
...
export function formatNotice(label, jobId, lines, maxBytes) {
  const head = `[hydrophone ${label} · ${jobId}] ${lines.length === 1 ? '1 line' : `${lines.length} lines`}:`
