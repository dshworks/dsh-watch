/**
 * dsh-hydrophone — background stream listeners that wake the agent.
 *
 * The `hydrophone` tool drops a listener on a long-running command or a
 ...
 *
 * Every listener is a first-class background job (kind `hydrophone`) started
...
export const name = 'dsh-hydrophone'
...
/**
 * Mount the `hydrophone` tool and its delivery plumbing.
...
export function apply(ctx, config) {
  ...
  ctx.systemPrompt.section({
    name: 'tool:hydrophone',
    order: 107,
    text: 'You can drop a hydrophone on a stream with the hydrophone tool: it listens in the background and wakes you when new matching lines arrive, so never busy-poll a source you already listen to. Listeners are background jobs — job_list shows them, job_output reads a listener\'s heard-line backlog, job_kill disarms one.',
  })
  ...
  function createListener(spec) {
    ...
    listener.timer = setInterval(() => {
      try {
        ...
        ctx.logger.warn(`dsh-hydrophone ${listener.jobId}: ${error instanceof Error ? error.message : String(error)}`)
      }
  ...
  function armCommand(args, exec, label, filter, maxEvents) {
    ...
    if (shell === undefined) {
      throw new Error('hydrophone: command sources need the shell capability (load @deepseek-ai/dsh-shell and a provider)')
    }
      ...
      run: () => {
        ...
        const listener = createListener({
          ...
          tick: (self) => {
            ...
            if (read.lossy) lines.push('[hydrophone: upstream output was truncated; some lines were lost]')
            emit(self, lines)
          },
        })
        return {
          cancel: () => {
            ...
            if (finalRead.lossy) lines.push('[hydrophone: upstream output was truncated; some lines were lost]')
  ...
  function commandOutcome(listener, proc) {
    ...
    if (listener.disarmed) return { status: 'killed', detail: 'hydrophone disarmed' }
    ...
    return ctx.jobs.start({
      ...
      run: () => {
        ...
        const listener = createListener({
          ...
          filter,
          maxEvents,
          onTeardown: () => settle({ status: 'killed', detail: 'hydrophone disarmed' }),
        ...
        return {
          cancel: () => {
            listener.disarmed = true
            listener.stop()
            settle({ status: 'killed', detail: 'hydrophone disarmed' })
          },
          done,
          readOutput: () => listener.backlog.drain(),
        }
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      ...
      render: (_args, value) => [{
        type: 'text',
        text: `Hydrophone armed (${value.job_id}) on ${value.source}: ${value.description}. `
    ...
    async execute(args, exec) {
      if (args.source === 'command' && (args.command === undefined || args.command === '')) {
        throw new Error('hydrophone: source "command" requires a non-empty command')
      }
      if (args.source === 'file' && (args.path === undefined || args.path === '')) {
        throw new Error('hydrophone: source "file" requires a non-empty path')
      }
      if (args.workdir !== undefined && args.source !== 'command') {
        throw new Error('hydrophone: workdir applies only to source "command"')
      }
      if (args.max_events !== undefined && (!Number.isSafeInteger(args.max_events) || args.max_events < 1)) {
        throw new Error(`hydrophone: max_events (${args.max_events}) must be a whole number of notices, at least 1`)
      }
      const filter = compileFilter(args.pattern)
      if (exec.signal.aborted) throw new Error('hydrophone: tool call aborted')
      if (exec.agent !== undefined) {
        let owned = 0
        for (const listener of active.values()) if (listener.owner === exec.agent) owned++
        if (owned >= config.maxListenersPerOwner) {
          throw new Error(`hydrophone: listener cap reached (${config.maxListenersPerOwner}); disarm one with job_kill first`)
        }
      }
